---
title: 观众渲染中的工作记录
published: 2025-05-06T18:36:36.000Z
...
## 1. 整体效果
注意场上又不同类型的观众，包含男女，带帽长发等等，衣服配色也有差异。能响应场上得分事件，动作之间存在过渡。

<Video src="/videos/audience.mp4" title="观众系统一览" loop={true} />

## 2. 引言
 
大规模观众渲染移动端来说是一个不小的挑战
- 效果方面：观众配色需要适配30+场馆，实例数量在6000以上，观众响应进球，动作尽可能多样且不重复，光照需要层次感。
...
最后是关于每个实例的世界空间位置信息，因为是使用到Unity中 `RenderMeshInstanced` 这个接口，所以需要每一帧传入每个实例的世界空间位置矩阵
将所有矩阵按照instance ID的顺序排列，组合在一起传入gpu后供每个实例读取。
- 有关观众位置生成的方法，简单来说是根据座椅的uv中心点生成cube或者是地板的uv填满cube（就是在世界空间Mesh上填满cube），随后记录这些cube的位置和旋转，用于观众位置的输入。
...
<Video src="/videos/textureArrayColor.mp4" title="上色到纹理" loop={true} />
...
>
  ...
  <div slot="tab5">
    有个好玩的事情是，如果游戏运行帧数设定为120帧，而观众隔一帧绘制的话，观众将会变得透明。

    但是运行帧数一般很难设置稳定，效果还是没法稳定。
  </div>

  <div slot="tab6">
    观众亮度需要过渡明暗区域，最简单的方式是调整ambient：
    ```astro
    float3 ambient = SampleSH(i.worldNormal)* (_DarkScale / (i.yPos + _DarkScale));
    ```
    越低的观众，也就是越前排的观众越亮。
  </div>
